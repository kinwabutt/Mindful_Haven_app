...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadProfileImage() async {
    ...
    if (mounted) {
      setState(() => _imagePath = prefs.getString('img_${_auth.currentUser?.uid}'));
    }
  }
  ...
  Future<void> _pickImage() async {
    ...
    if (pickedFile != null) {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('img_${_auth.currentUser?.uid}', pickedFile.path);
      setState(() => _imagePath = pickedFile.path);
    }
  }

  // --- OPTIMIZED DELETE ACCOUNT LOGIC ---
  Future<void> _deleteUserAccount() async {
    ...
    try {
      ...
      // 1. Collect and Batch Delete Chat History
      var chatHistory = await _firestore.collection('chats').doc(uid).collection('history').get();
      ...
      // 2. Delete Profile and Root Chat Doc
      batch.delete(_firestore.collection('chats').doc(uid));
      batch.delete(_firestore.collection('users').doc(uid));

      // Commit all Firestore deletes at once
      await batch.commit();
      ...
      if (e.code == 'requires-recent-login') {
        _showErrorSnackBar("Security check: Please logout and login again to delete your account.");
      } else {
                ...
                return SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      Stack(
                        ...
                        children: [
                          CircleAvatar(
                            ...
                            child: _imagePath == null ? const Icon(Icons.person, size: 60, color: primaryTeal) : null,
                          ),
                          GestureDetector(
                            onTap: _pickImage,
                      ...
                      Text(bio, style: TextStyle(color: isDark ? Colors.white70 : Colors.grey[600])),
                      ...
                      if (level.isNotEmpty || institute.isNotEmpty)
                        Container(
                          ...
                          child: Column(
                            children: [
                              if (level.isNotEmpty) _infoRow(Icons.school_outlined, "Grade / Level", level, isDark),
                              if (level.isNotEmpty && institute.isNotEmpty) const Divider(height: 30),
                              if (institute.isNotEmpty) _infoRow(Icons.account_balance_outlined, "Institute", institute, isDark),
                            ],
                          ),
                        ),
                      
                      const SizedBox(height: 25),
                      _menuItem(Icons.edit_note_rounded, "Edit Profile", () => _showEditSheet(data, isDark), isDark),
                      _menuItem(Icons.settings_suggest_rounded, "App Settings", _showSettingsDialog, isDark),
                      _menuItem(Icons.shield_moon_outlined, "Emergency Support", () => Navigator.push(context, MaterialPageRoute(builder: (context) => const EmergencySupportScreen())), isDark),
                      _menuItem(Icons.emergency_share_outlined, "Emergency Settings", () => Navigator.push(context, MaterialPageRoute(builder: (context) => const EmergencySettingsScreen())), isDark),

                      const SizedBox(height: 30),
                      TextButton.icon(
                        onPressed: _confirmDeletion,
                        icon: const Icon(Icons.delete_forever_rounded, color: Colors.redAccent),
                        label: const Text("Delete Account & Data", style: TextStyle(color: Colors.redAccent, fontSize: 14)),
                      ),
                      TextButton.icon(
                        onPressed: () async {
  ...
  Widget _infoRow(IconData icon, String label, String value, bool isDark) => Row(
    children: [
      Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFF26C6DA).withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
      ...
      Column(
        ...
        children: [
          ...
          Text(value, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: isDark ? Colors.white : Colors.black)),
        ],
      ),
    ],
  );

  Widget _menuItem(IconData icon, String title, VoidCallback onTap, bool isDark) => InkWell(
    onTap: onTap,
    ...
    child: Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 15),
          Text(title, style: TextStyle(color: isDark ? Colors.white : Colors.black, fontWeight: FontWeight.w600, fontSize: 15)),
          const Spacer(),
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _textField(instC, "Institute", isDark, Icons.account_balance_outlined),
            const SizedBox(height: 20),
            ElevatedButton(
  ...
  Widget _textField(TextEditingController controller, String label, bool isDark, IconData icon) => Padding(
      ...
      builder: (_) => StatefulBuilder(
        builder: (context, setDialogState) {
          ...
          return AlertDialog(
            ...
            title: Text("App Settings", style: TextStyle(color: currentIsDark ? Colors.white : Colors.black)),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                SwitchListTile(
                  title: Text("Dark Mode", style: TextStyle(color: currentIsDark ? Colors.white : Colors.black)),
                  value: currentIsDark,
